  ...
  return (
    <header
      className={`fixed top-0 inset-x-0 z-50 transition-all duration-500 ease-cinematic ${
        scrolled ? "py-3 bg-ink/80 backdrop-blur-md border-b border-ink-line" : "py-6 bg-transparent"
      }`}
    >
      <nav className="mx-auto max-w-shell px-6 md:px-10 flex items-center justify-between">
        ...
        <a 
          ...
          data-cursor="link" 
          className="flex items-center gap-2.5 font-display font-semibold text-lg tracking-tight text-white hover:text-[#E1306C] transition-colors duration-300"
        >
        ...
        <ul className="hidden md:flex items-center gap-9 text-sm text-paper-dim">
          {nav.map((item) => (
            <li key={item.href}>
              <a href={item.href} data-cursor="point" className="hover:text-paper transition-colors duration-200">
                {item.label}
              </a>
            </li>
          ))}
        ...
        <a
          ...
          rel="noopener noreferrer"
          data-cursor="link"
          className="hidden md:inline-flex items-center gap-2 rounded-full border border-ink-line px-5 py-2.5 text-sm text-paper hover:border-[#5865F2] hover:bg-[#5865F2] hover:text-white transition-all duration-300"
        >
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            <ul className="flex flex-col px-6 py-6 gap-5 text-lg">
              {nav.map((item) => (
                <li key={item.href}>
                  <a href={item.href} onClick={() => setOpen(false)} data-cursor="point" className="text-paper">
                    {item.label}
                  </a>
                </li>
              ))}
              ...
              <li>
                <a 
                  ...
                  data-cursor="link" 
                  className="flex items-center gap-3 text-[#5865F2]"
                >
                  <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.445.865-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.618-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.319 13.58.099 18.058a.082.082 0 0 0 .031.056 19.908 19.908 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028 14.09 14.09 0 0 0 1.226-1.994.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.892.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.029zM8.02 15.33c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.956-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.956 2.418-2.157 2.418zm7.975 0c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.955-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.946 2.418-2.157 2.418z" />
                  </svg>
